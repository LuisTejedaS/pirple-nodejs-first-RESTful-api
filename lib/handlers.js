...
handlers.index = function (data, callback) {
    if (data.method == 'get') {
        ...
        var templateData = {
            ...
            'head.description': 'We ofer free, simple uptime monitoring for HTTP/HTTPS sites of all kinds',
            'body.class': 'index'
        };
...
handlers.accountCreate = function(data, callback){
    if (data.method == 'get') {
        ...
        var templateData = {
            ...
            'head.description': 'Sing up is easy and only takes a few seconds',
...
handlers.sessionCreate = function(data, callback){
    if (data.method == 'get') {
        ...
        var templateData = {
            ...
            'head.description': 'Pleasse enter your phone number and password to acess',
...
handlers.sessionDeleted = function(data, callback){
    if (data.method == 'get') {
        ...
        var templateData = {
            ...
            'head.description': 'You hae been logged out',
    ...
    if (firstName && lastName && phone && tosAgreement) {
        _data.read('users', phone, function (err, data) {
            if (err) {
                ...
                callback(400, {
                    'Error': 'A user with that number already exists'
                });
            }
        });
    } else {
                                        ...
                                        _data.delete('checks', checkID, function (err) {
                                            ...
                                            if (checksDeleted == checksToDelete) {
                                                if (!deletionErrors) {
                                                    ...
                                                    callback(500, {
                                                        'Error': 'Errors encountered while attempting to delete all of the users checks'
                                                    });
                                                }
                                            }
                                        });
        ...
        _data.read('users', phone, function (err, userData) {
            if (!err && userData) {
                ...
                if (hashedPassword == userData.hashedPassword) {
                    ...
                    callback(400, {
                        'Error': 'Password did not match the specified user\s stored passowrod'
                    });
                }
            } else {
                callback(400, {
...
handlers._tokens.put = function (data, callback) {
    ...
    if (id && extend) {
        ...
        callback(400, {
            'Error': 'Missing required fields or fields ar invalid'
        });
    }
...
handlers._checks.put = function (data, callback) {
    ...
    if (id) {
        if (protocol || url || method || succesCodes || timeoutSeconds) {
            ...
            callback(400, {
                'Error': 'Missing required fields to update'
            });
        }
    ...
    if (id) {
        _data.read('checks', id, function (err, checkData) {
            if (!err && checkData) {
                                ...
                                _data.read('users', checkData.userPhone, function (err, userData) {
                                    if (!err && userData) {
                                        ...
                                        if (checkPosition > -1) {
                                            ...
                                            callback(500, {
                                                'Error': 'Could not find the check on the user\'s object, so could not be removed'
                                            });
                ...
                callback(400, {
                    'Error': 'could not find the specified check'
                });
            }
        });
    } else {
